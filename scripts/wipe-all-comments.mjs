...
async function deleteAllSupabaseComments() {
  const supabase = getSupabase();
  let totalDeleted = 0;
  let attempts = 0;
  const maxAttempts = 2000; // Aumentar limite
  
  console.log('🗑️ Deletando comentários do Supabase em lotes pequenos...');
...
async function main() {
  ...
  if (supabaseCount > 0) {
    console.log('\n🗑️ Limpando Supabase...');
    const deleted = await deleteAllSupabaseComments();
