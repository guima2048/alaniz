...
async function deleteCommentsInBatchesOf1000() {
  const supabase = getSupabase();
  let totalDeleted = 0;
  let batchCount = 0;
  
  console.log('🗑️ Deletando comentários em lotes de 1000...');
  
  while (true) {
    // Buscar um lote de 1000 IDs
...
async function main() {
  ...
  if (supabaseCount > 0) {
    console.log('\n🗑️ Limpando Supabase em lotes de 1000...');
    const deleted = await deleteCommentsInBatchesOf1000();
